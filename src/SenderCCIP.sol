...
contract SenderCCIP is Ownable, CCIPReceiver {
    ...
    constructor(address _routerClientAddressMainChain, address _usdc, address _link, uint64 _main_chain_selector)
        Ownable(msg.sender)
        CCIPReceiver(_routerClientAddressMainChain)
    {
    ...
    function sendEntryCrossChain(uint256 _nbTickets, address _player, uint256 _totalCost)
        internal
        returns (bytes32 messageID)
    {
        bytes memory functionCallData =
            abi.encodeWithSelector(IRaffle.enterRaffleCrossChain.selector, _nbTickets, _player, _totalCost);
